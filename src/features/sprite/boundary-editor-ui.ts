...
export function readBoundaryEditorValues(
  ...
  const readEdge = (input: HTMLInputElement): number | null => {
    const parsed = Number.parseInt(input.value, 10);
    return Number.isFinite(parsed) ? parsed : null;
  };
  const left = readEdge(els.boundaryLeft);
  const top = readEdge(els.boundaryTop);
  const right = readEdge(els.boundaryRight);
  const bottom = readEdge(els.boundaryBottom);
  ...
  const anchorLocal = readEdge(els.boundaryAnchorX);
  if (anchorLocal === null) {
    return null;
  }
