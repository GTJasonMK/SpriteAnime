...
export interface SpriteFrameViewState {
...
export class SpriteFrameController {
  ...
  getWorkspaceState(): SpriteFrameViewState {
    return {
      selectedIndices: [...this.selectedIndices],
      currentFramePosition: this.currentFrame,
      fps: this.fps,
      scale: this.scale,
    };
  }

  restoreWorkspaceState(state: SpriteFrameViewState): void {
    if (!Number.isInteger(state.fps) || state.fps < 1 || state.fps > 60) {
      throw new Error("序列帧播放 FPS 快照无效");
    }
    if (!Number.isFinite(state.scale) || state.scale < 0.25 || state.scale > 3) {
      throw new Error("序列帧播放缩放快照无效");
    }
    const unique = new Set(state.selectedIndices);
    if (
      unique.size !== state.selectedIndices.length ||
      state.selectedIndices.some((index) => !Number.isInteger(index) || index < 0 || index >= this.frames.length)
    ) {
      throw new Error("序列帧选择快照包含无效帧索引");
    }
    if (
      !Number.isInteger(state.currentFramePosition) ||
      state.currentFramePosition < 0 ||
      (state.selectedIndices.length > 0 && state.currentFramePosition >= state.selectedIndices.length) ||
      (state.selectedIndices.length === 0 && state.currentFramePosition !== 0)
    ) {
      throw new Error("序列帧当前播放位置快照无效");
    }
    this.selectedIndices = [...state.selectedIndices];
    this.els.fpsSlider.value = String(state.fps);
    this.els.scaleSlider.value = String(state.scale);
    this.setFps(state.fps);
    this.setScale(state.scale);
    this.updateThumbnailStates();
    this.currentFrame = state.currentFramePosition;
  ...
  async applyFrames(
    ...
    this.els.frameSizeInfo.textContent =
      `已拆分: ${rows}行 x ${cols}列 | 单帧: ${frame.width}x${frame.height} | ${sourceInfo} | 定位针对齐`;
