...
export function resetFrameRegion(frame: FrameBounds, empty: boolean): void {
  const region = defaultFrameRegion(frame);
  frame.x = region.x;
  frame.y = region.y;
  frame.width = region.width;
  frame.height = region.height;
  frame.anchorX = getRegionCenterX(region.x, region.width);
  frame.empty = empty;
}
...
export function recalculateAutoBounds(bounds: AutoBoundsResult): void {
  ...
  if (emptyCount === bounds.frameBounds.length) {
    bounds.fixedOffsetX = 0;
    bounds.fixedOffsetY = 0;
    bounds.fixedWidth = first.cellWidth;
    bounds.fixedHeight = first.cellHeight;
