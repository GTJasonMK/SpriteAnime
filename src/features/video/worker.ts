...
import { clampPixelBounds, expandPixelBounds } from "./utils";
...
async function processFrames(message: ProcessMessage): Promise<void> {
  try {
    ...
    const sheetBlob = await composeSpriteSheet(processed, message.options);
    self.postMessage({
      id: message.id,
      type: "success",
...
function buildExtractedFrame(
  ...
  const bgRgb = options.bgMode === "edge"
    ...
    : null;
  const shouldDetectForeground = options.autoTrim && options.bgMode !== "none";
  const foregroundMask = shouldDetectForeground && options.transparent
    ? new Uint8Array(imageData.width * imageData.height)
    : null;
  const detected = shouldDetectForeground
    ? scanForegroundBounds(imageData, firstFrameData, bgRgb, options, foregroundMask)
    : null;
...
async function composeSpriteSheet(
  ...
  const cellWidth = frames.reduce((max, frame) => Math.max(max, frame.canvas.width), 1);
  const cellHeight = frames.reduce((max, frame) => Math.max(max, frame.canvas.height), 1);
  ...
  frames.forEach((frame, index) => {
    ...
    const x = col * cellWidth + Math.round((cellWidth - frame.canvas.width) / 2);
    const y = row * cellHeight + (cellHeight - frame.canvas.height);
