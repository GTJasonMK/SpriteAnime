...
export function renderGeneratedGallery(options: {
  ...
  records.forEach((record, index) => {
    ...
    setImageSrcWithSingleRetry(img, record.path);
...
export function setSelectedGeneratedPreview(options: {
  ...
  const { image, meta, record, formatTime, formatDuration } = options;
  if (!record) {
  ...
  setImageSrcWithSingleRetry(image, record.path);
  meta.textContent =
    `${record.label} · ${record.model || "未知模型"} · ${formatTime(record.createdAt)} · 耗时 ${formatDuration(record.durationSeconds)}`;
}

function setImageSrcWithSingleRetry(img: HTMLImageElement, path: string): void {
